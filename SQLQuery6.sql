...
WHILE @Counter < @NumberOfGuests
BEGIN
    SET @CheckInDate = DATEADD(DAY, -ROUND(RAND() * @MaxBookingDateDiff), GETDATE());
    SET @CheckOutDate = DATEADD(DAY, ROUND(RAND() * @MaxBookingDateDiff), @CheckInDate);

    INSERT INTO Rezervasyonlar (RezervasyonID, MisafirID, OdaID, GirisTarihi, CikisTarihi)
    VALUES (@Counter + 1, ROUND(RAND() * @NumberOfGuests) + 1, ROUND(RAND() * @NumberOfRooms) + 1, @CheckInDate, @CheckOutDate);
